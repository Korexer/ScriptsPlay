...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-8 pb-12">
      {/* Welcome Panel */}
      <div className="bg-gradient-to-r from-[#0f172a] to-[#1e3a8a] rounded-3xl p-8 md:p-10 shadow-xl shadow-blue-900/10 text-white relative overflow-hidden group">
        ...
        <div className="relative z-10">
          <h1 className="text-3xl md:text-4xl font-['Outfit'] font-bold mb-2 tracking-tight">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <span className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-white/10 border border-white/20 backdrop-blur-md">
              <Star className="w-4 h-4 mr-2 text-yellow-500 fill-yellow-500" />
              Role: {workerData?.role === 'transcriber' ? 'Transcriber' : (workerData?.role || 'Entry Level')}
            </span>
            {workerData?.access_granted ? (
              <span className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-green-500/20 text-green-300 border border-green-500/30 backdrop-blur-md">
                <CheckCircle2 className="w-4 h-4 mr-2" />
                Access Status: Active
              </span>
            ) : (
              <span className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-red-500/20 text-red-300 border border-red-500/30 backdrop-blur-md">
      ...
      {!workerData?.access_granted && (
        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm relative overflow-hidden">
          ...
          <Link to="/spworkersportal101/unlock-access" className="whitespace-nowrap inline-flex items-center justify-center px-5 py-2.5 rounded-xl font-medium text-white bg-orange-500 hover:bg-orange-600 shadow-sm shadow-orange-500/30 transition-all hover:-translate-y-0.5 shrink-0">
            Unlock Now
          </Link>
        </div>
      )}
      ...
      {/* Stats */}
      <h2 className="text-2xl font-bold font-['Outfit'] text-slate-900 mb-4 px-1">Worker Stats</h2>
      ...
      {/* Quick Links */}
      <div className="pt-4">
        <h2 className="text-2xl font-bold font-['Outfit'] text-slate-900 mb-4 px-1">Quick Actions</h2>
